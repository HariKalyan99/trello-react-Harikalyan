...
const TrelloCheckListSpace = ({name, id, deleteCheckList}) => {
...
const [getDelCheckItem, setDelCheckItem] = useState("");
  ...
  useEffect(() => {
    const getCheckItem = async(id) => {
        try {
            const {data} = await axios.get(`https://api.trello.com/1/checklists/${id}/checkItems?key=${APIKey}&token=${APIToken}`);
            if(data?.length > 0){
                setCheckItems(data);
            }
        } catch (error) {
  ...
  useEffect(() => {
      const postCheckItem = async({id, name}) => {
        try {
          const {data} = await axios.post(`https://api.trello.com/1/checklists/${id}/checkItems?name=${name}&key=${APIKey}&token=${APIToken}`)
  ...
  useEffect(() => {
    const deleteCheckItem = async(idCheckItem) => {
      try {
        await axios.delete(`https://api.trello.com/1/checklists/${id}/checkItems/${idCheckItem}?key=${APIKey}&token=${APIToken}`)
      ...
      <Row className="w-[400px]">
        <Col span={24}>
          <Slider
            ...
            value={checkItems?.filter(x => x.state === "complete")?.length + 1}
          />
        </Col>
      </Row>

      {checkItems?.length > 0 && checkItems?.map(({id, name, state}, ind) => <CheckItem  key={id} state={state} deleteCheckItem={deleteCheckItem} name={name} id={id} />)}
     {displayCheck ?   <Space >
        <form id="checkItemForm" className="flex flex-col justify-center items-start gap-3" onSubmit={(e) => {
            e.preventDefault();
            addCheckItem({name: checkItemRef.current.value, id});
            checkItemRef.current.value = "";
            setDisplayCheck(!displayCheck);
        }}>
        <input type="text" className="w-[100%] p-3 border-4" placeholder="Add a check item" ref={checkItemRef}/>
      ...
      >
        ...
        <MdOutlineCancel  onClick={() => setDisplayCheck(!displayCheck)} className="text-red-300 hover:text-red-400 text-xl"/> 
      </Space>
        </form>
